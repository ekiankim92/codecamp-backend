...
export class ProductsService {
  ...
  async create({
    ...
    // 2-2) 상품태그 등록
    // productTags 가 ["#전자", "#영등포", "#컴퓨터"] 와 같은 패턴이라고 가정
    const tagNames = productTags.map((el) => el.replace('#', '')); // ["전자", "영등포", "컴퓨터"]
    ...
    // 안좋은 방법 round trip time db 에 연속적으로 접근
    // for (let i=0; i <=2; i++) {
    //   this.productTagsRepository.save({name: tagNames[i]})
    // }
  ...
  }: IUpdateProductInput): Promise<void> {
